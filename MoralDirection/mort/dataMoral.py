...
actions = [
    ...
    ("smile", "at a murderer,at a stranger,at my friend"),
    ...
    ("go", "to church,to work,to the cinema"),
...
def get_actions_projection(lang="en"):
    """ so-called context list in get_emb_cluster_pca """
    out = list()
    ...
    if lang == "en":
        ...
        return get_translated_qs(lang, out)


def get_actions_verbs_projection(lang="en"):
    """ so-called atomic list in get_emb_cluster_pca """
    out = list()
...
def get_translated_qs(lang, out) -> Dict[str, List[str]]:
    file_loc = os.path.dirname(os.path.realpath(__file__))
    translated_out = {}
    translations_sorted_file = "../data/translated_questions_sorted.json"
    translations_sorted_file = os.path.normpath(os.path.join(file_loc, translations_sorted_file))
    with open(translations_sorted_file, "r", encoding="utf-8") as translations_file:
        translations = json.load(translations_file)
        for phrase in out:
            try:
                translated_out[phrase] = translations[lang][phrase]
            except KeyError as e:
                print("Warning: Missing verb/phrase", e)
...
def adaptbias_get_actions_adapt_moral():
    data = np.genfromtxt(os.path.join(_working_path, 'data/parsed_yes_no_BERTBias.csv'), skip_header=1, dtype=str,
                         delimiter=",")
    res = dict()
    labels = dict()
    template_ = [quest_answ_triple[0] for quest_answ_triple in experimental_quests_paper]
...
def enumerate_questions():
    all_qs: List[List[str, str, str]] = []
    with open("../data/verb-phrase-collection.txt", "r", encoding="utf-8") as infile:
        phrases = [line.strip() for line in infile.readlines()]
    for i, phrase in enumerate(phrases):
        for q in experimental_quests_paper:
            all_qs.append([str(q[0].format(phrase)), q[1], q[2]])
    with open("../data/enumerated-questions.csv", "w+", encoding="utf-8") as outfile:
        writer = csv.writer(outfile)
        for q in all_qs:
            writer.writerow(q)


def sort_translated_qs(overwrite: bool = True):
    num_questions = 10
    with open("../data/verb-phrase-collection.txt", "r", encoding="utf-8") as infile:
        phrases_en = [line.strip() for line in infile.readlines()]
    num_neg_answers = len(phrases_en * num_questions)
    translated_qs = {}
    for lang in "ar de cs zh".split():
        translated_qs[lang] = {}
        for phrase in phrases_en:
            translated_qs[lang][phrase] = []
        question_mark = "？" if lang == "zh" else ("؟" if lang == "ar" else "?")
        with open(f"../data/translation_enumerated-questions/{lang}.txt", "r", encoding="utf-8") as infile:
            translated_lines = infile.readlines()
        for j in range(num_neg_answers):
            line = translated_lines[j]
            phrase_idx = j // num_questions
            line = line.strip().split(question_mark)
            q = line[0] + question_mark
            neg_answer = line[1]
            pos_answer = translated_lines[j + num_neg_answers].strip().split(question_mark)[1]
            phrase_en = phrases_en[phrase_idx]
            translated_qs[lang][phrase_en].append([q, pos_answer, neg_answer])

    translations_sorted_file = "../data/translated_questions_sorted.json"
    if not os.path.isfile(translations_sorted_file) or overwrite:
        with open(translations_sorted_file, "w+", encoding="utf-8") as outfile:
            json.dump(translated_qs, outfile)
