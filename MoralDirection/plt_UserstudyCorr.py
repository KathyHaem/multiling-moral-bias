...
# input("Press key")
def _corr(args, dos, donts, model_name, text_pos, user_study: str):
    model_csv_file = "data/correlation/userstudy/{}_{}_{}_{}_bias_{}.csv".format(
        args.postproc, args.lang, model_name, args.model_name, user_study)
    # read dos based on input does
    with open(model_csv_file, 'r', encoding='utf-8', newline='') as f:
        reader = csv.reader(f, delimiter=',')
        d = [x for x in list(reader) if x]  # ignores spurious blank lines that seem to happen on Windows
        try:
            tmp = float(d[-1][0])
            dim_score = 0
            dim_action = 1
    ...
    data_dos = sort_dos_donts(data_mcm, dim_action, dim_score, dos)
    ...
    data_donts = sort_dos_donts(data_mcm, dim_action, dim_score, donts)
    ...
    own_plot(x, y, a, b, suffix="{}_{}_{}userstudy_vs_{}_{}".format(
        args.postproc, args.lang, user_study, model_name, args.model_name), text_pos=text_pos)
...
def run_correlations(args, user_study):
    with open(f"data/correlation/userstudy/userStudy_scores_{user_study}.csv", 'r', encoding='utf-8', newline='') as f:
        reader = csv.reader(f, delimiter=',')
        data = list(reader)
    ...
    print("BERT subspace")
    text_pos = (-0.25, 1.2,)
    _corr(args, dos, donts, f'BERT_subspace_qa', text_pos, user_study)
