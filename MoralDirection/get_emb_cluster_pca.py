...
from mort.representation_postproc import proc_corpus_sample
from mort.dataMoral import get_actions_temporal, get_actions_verbs_projection, get_actions_projection, \
    experimental_quests_paper, get_actions_verbs_allBooksNews
...
def get_corpus_embeddings_PCA(args, get_sen_embedding_, data_verbs):
    ...
    if args.lang == "en":
        for q_idx in tqdm(range(len(questions))):
            ...
            res_tmp = list()
            for i in tqdm(range(int(len(data) // batch_size) + 1)):
                batch = data[i * batch_size: i * batch_size + batch_size]
                res_tmp += get_sen_embedding_(batch, dtype='list')
            res_tmp = np.array(res_tmp)
            assert len(res_tmp) == len(data)
            res.append(res_tmp)
        res = np.stack(res)
    else:
        verb_order = []
        for verb_en, translated_qs in tqdm(data_verbs.items()):
            verb_order.append(verb_en)
            only_qs = [q[0] for q in translated_qs]
            res_tmp = get_sen_embedding_(only_qs)
            res.append(res_tmp)
        res = np.stack(res).swapaxes(0, 1)
        data_verbs = verb_order

    all_sent_embeds = res.reshape([len(questions) * len(data_verbs), -1])
    # options: "standardize" / "rm_mean" / "rm_top5_dims" / "rm_top_pcs"
    sents_postproc = proc_corpus_sample(sample_embeds=all_sent_embeds, postproc=args.postproc)
    mean = all_sent_embeds.mean(axis=0)
    stdev = all_sent_embeds.std(axis=0)
    res = sents_postproc.reshape([len(questions), len(data_verbs), -1])
    # average over all questions for each phrase
    res = res.mean(0)
    ...
    return corpus_embeddings, dim_reduc, Y, mean, stdev
...
def main():
    ...
    transformer_model = args.bert_model_name
    transformer_model_name = os.path.basename(transformer_model) if os.path.isdir(
        transformer_model) else transformer_model

    filename = 'mort/results/bias/{}_{}_{}Verbs_{}Embedding_{}.csv'.format(
        args.postproc,
        args.lang,
        args.data_cluster,
        args.model,
    ...
    if args.model == 'use_hub':
        ...
        emb = BERTSentence(device="cpu", transformer_model=transformer_model)
        get_sen_embedding_ = emb.get_sen_embedding

    if True: # not os.path.isfile(filename_pickled_cluster):
        # data = get_actions_verbs()
        if args.data_cluster == 'context':
            data_verbs = get_actions_projection(lang=args.lang)
        elif args.data_cluster == 'atomic':
            data_verbs = get_actions_verbs_projection(lang=args.lang)
        ...
        corpus_embeddings, dim_reduc, Y, mean, stdev = get_corpus_embeddings_PCA(args, get_sen_embedding_, data_verbs)
        action_colors_dict, clustered_sentences = create_sentence_clusters(args, corpus_embeddings)

        # clustered_sentences.sort(key=lambda x: -x[1])
        save_projection_model(Y, action_colors_dict, clustered_sentences, corpus_embeddings,
                              dim_reduc, mean, stdev, filename_pickled_cluster)
    else:
        ...
        mean = moralprojection_model["mean"]
        stdev = moralprojection_model["std"]
        actions_colors = moralprojection_model["actions_colors"]
        corpus_embeddings = moralprojection_model["corpus_embeddings"]

    norm = plot_individual_verbs_projection(Y, corpus_embeddings, filename_pickled_cluster)
    plot_variance_ratio(dim_reduc, filename_pickled_cluster)

    verbose = False

    if args.data == 'context':
        test_actions = get_actions_projection(lang=args.lang)
    elif args.data == 'atomic':
        test_actions = get_actions_verbs_projection(lang=args.lang)
    elif args.data == 'allNews':
        test_actions = get_actions_temporal(args.data)
    elif args.data == 'all':
        test_actions = get_actions_verbs_allBooksNews()
    else:
        raise ValueError("data not found: {}".format(args.data))
    ...
    for test_action in tqdm(test_actions):
        if args.lang == "en":
            ...
            test_questions = test_actions[test_action]
            test_questions = [q[0] for q in test_questions]
            actions_order.append(test_action)
        test = get_sen_embedding_(test_questions, dtype='list')
        test_embeds.append(test)
    test_actions = test_actions if args.lang == "en" else actions_order
    test_embeds = np.stack(test_embeds).reshape([len(experimental_quests_paper) * len(test_actions), -1])
    test_embeds = proc_corpus_sample(
        test_embeds, postproc=args.postproc  # , means=mean, stds=stdev  - should this get exact same transforms?
    )

    test_embeds = test_embeds.reshape([len(test_actions), len(experimental_quests_paper), -1])
    for test, test_action in zip(test_embeds, test_actions):
        ...
        plt.scatter([test_plot[0]], [y_pos], c=[colors_a[0] if test_plot[0] <= 0 else colors_a[1]],
                    marker="x")  # test_plot[1] just for viz
    ...
    if True:
        save_csvs(Y, args, corpus_embeddings, res, transformer_model_name)


def save_csvs(Y, args, corpus_embeddings, res, transformer_model_name):
    # save data to file

    # file with phrases
    filename = 'mort/results/bias/cluster/{}_{}_{}Verbs_{}_Embedding_{}_Cluster{}_Embdim{}{}Verbs_PCA.csv'.format(
        args.postproc,
        args.lang,
        args.data,
    ...
    res.sort(key=lambda x: x[1])
    # file with individual verbs
    # this seems to be same data as moral_projection.csv?
...
def save_projection_model(Y, action_colors_dict, clustered_sentences, corpus_embeddings, dim_reduc, mean, stdev,
                          filename_pickled_cluster):
    actions_colors = list()
    ...
    moralprojection_model["mean"] = mean
    moralprojection_model["std"] = stdev
...
def plot_variance_ratio(dim_reduc, filename_pickled_cluster):
...
def plot_individual_verbs_projection(Y, corpus_embeddings, filename_pickled_cluster):
    # plt.show()
...
def create_sentence_clusters(args, corpus_embeddings):
    ...
    data_verbs = corpus_embeddings[1]
    ...
    return action_colors_dict, clustered_sentences
...
def parse_args():
    ...
    parser.add_argument("--postproc", default=None, type=str,
                        help="Post-processing function to apply. "
